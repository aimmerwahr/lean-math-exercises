...
/-- **Question 1.** Bézout's identity expresses a Euclidean gcd as a linear combination. -/
theorem q1_bezout {R : Type*} [EuclideanDomain R] [DecidableEq R] (a b : R) :
    ∃ x y : R, EuclideanDomain.gcd a b = a * x + b * y := by sorry


/-- **Question 2.** In a unique factorization domain, irreducible and prime elements coincide. -/
...
/-- **Question 3.** Euclidean division decomposes a dividend into quotient times divisor plus
remainder. -/
theorem q3_division_identity {R : Type*} [EuclideanDomain R] (a b : R) :
    a % b + b * (a / b) = a := by sorry


/-- **Question 4.** A polynomial ring over a field has no zero divisors (indeed it is Euclidean). -/
theorem q4_polynomial_domain (K : Type*) [Field K] : IsDomain K[X] := by sorry


/-- **Question 5.** Eisenstein's criterion turns a primitive polynomial with the required
coefficient divisibility into an irreducible polynomial. -/
...
/-- **Question 6.** The square root of two is irrational. -/
theorem q6_sqrt2_irrational : Irrational (Real.sqrt 2) := by sorry


/-- **Question 7.** The Euclidean gcd of `12` and `18` is `6`. -/
theorem q7_gcd_12_18 : EuclideanDomain.gcd (12 : ℤ) 18 = 6 := by sorry


/-- **Question 8.** In `ℤ[√−5]`, `6 = 2·3 = (1 + √−5)(1 - √−5)`, the equality underlying the
classical non-unique-factorization example. -/
