import Exercises.LinearAlgebra.«03Basis»
import Solutions.LinearAlgebra.«03Basis»
...
reader's own attempts in `Exercises/LinearAlgebra/03Basis.lean` (the build fails here if a proof
uses a banned lemma) and the shipped canonical proofs in `Solutions/LinearAlgebra/03Basis.lean`
