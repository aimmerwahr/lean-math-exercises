...
theorem q9_order_eight_pairwise_not_equiv :
  ...
  have kill_two : ∀ x : ZMod 2 × ZMod 2 × ZMod 2, (2 : ℕ) • x = 0 := by
  ...
  · rintro ⟨e⟩
    have hdiv : addOrderOf (1 : ZMod 8) ∣ 2 := by
      rw [← e.addOrderOf_eq]
      exact addOrderOf_dvd_iff_nsmul_eq_zero.mpr (kill_two (e 1))
    rw [ZMod.addOrderOf_one] at hdiv
    norm_num at hdiv
  · rintro ⟨e⟩
    have hdiv : addOrderOf ((1, 0) : ZMod 4 × ZMod 2) ∣ 2 := by
      rw [← e.addOrderOf_eq]
      exact addOrderOf_dvd_iff_nsmul_eq_zero.mpr (kill_two (e (1, 0)))
    norm_num [Prod.addOrderOf_mk, ZMod.addOrderOf_one] at hdiv
