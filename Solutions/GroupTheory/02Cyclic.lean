...
theorem q1_orderOf_pow [Finite G] (g : G) (k : ℕ) :
...
theorem q2_generator_iff_coprime [Finite G] (g : G) (k : ℕ) :
  ...
  rw [q1_orderOf_pow g k, Nat.div_eq_self]
  constructor
  · rintro (h | h)
...
theorem q5_unique_subgroup_per_divisor [Finite G] [IsCyclic G] {d : ℕ}
  ...
  have hchar : ∀ H : Subgroup G, H = Subgroup.zpowers (g ^ (Nat.card G / Nat.card H)) := by
    ...
    have hcard : Nat.card H = Nat.card G / Nat.gcd (Nat.card G) k := by
      rw [hk, Nat.card_zpowers, q1_orderOf_pow g k, hgord]
  ...
  · show Nat.card (Subgroup.zpowers (g ^ (Nat.card G / d))) = d
    rw [Nat.card_zpowers, q1_orderOf_pow g (Nat.card G / d), hgord]
